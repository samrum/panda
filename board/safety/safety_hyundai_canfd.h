...
static void hyundai_canfd_rx_hook(CANPacket_t *to_push) {
  ...
  const int pt_bus = hyundai_canfd_hda2 ? 1L : 0L;
  ...
  if (bus == pt_bus) {
    ...
    // cruise buttons
    const int button_addr = hyundai_canfd_alt_buttons ? 0x1aafffff : 0x1cffffff;
    if (addr == button_addr) {
      int main_button = 0;
      int cruise_button = 0;
      if (addr == 0x1cf) {
  ...
  if (hyundai_longitudinal) {
    ...
    const int stock_scc_bus = hyundai_canfd_hda2 ? 1L : 0L;
...
static int hyundai_canfd_fwd_hook(int bus_num, int addr) {
  ...
  if (bus_num == 2) {
    ...
    int hda2_lfa_block_addr = hyundai_canfd_hda2_alt_steering ? 0x362fffff : 0x2a4fffff;
